...
pub async fn import_obsidian_vault(
	...
	if dry_run {
		...
		stats.relationships_created = imported_notes
			.iter()
			.map(|n| n.wikilinks.len())
			.sum();
		return Ok(stats);
	}
...
mod tests {
	...
	fn test_folder_tags() {
		...
		assert_eq!(
			tags,
			vec!["folder:projects", "folder:projects/foo"]
		);
	}
	...
	fn test_folder_tags_deep() {
		...
		assert_eq!(
			tags,
			vec!["folder:a", "folder:a/b", "folder:a/b/c"]
		);
	}
	...
	fn test_determine_kind_event_from_date() {
		let mut fm = HashMap::new();
		fm.insert("date".into(), serde_json::Value::String("2024-01-15".into()));

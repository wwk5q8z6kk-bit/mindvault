...
pub fn list_prompts() -> Vec<PromptDefinition> {
	vec![
		...
		PromptDefinition {
			...
			description: "Generate a daily briefing based on recent vault activity, due tasks, and insights.".into(),
			arguments: vec![PromptArgument {
				name: "namespace".into(),
...
pub async fn get_prompt(
	...
	match name {
		"summarize" => prompt_summarize(engine, args).await,
		"extract_tasks" => prompt_extract_tasks(engine, args).await,
		"daily_briefing" => prompt_daily_briefing(engine, args).await,
			...
			let ctx_parts: Vec<String> = results
				...
				.map(|r| {
					let title = r
						...
						.as_deref()
						.unwrap_or("(untitled)");
					format!("- [{} | {}] {}: {}", r.node.kind, title, r.node.namespace, truncate(&r.node.content, 300))
				})
				.collect();

...
impl McpServer {
	...
	pub async fn run_stdio(&self) -> Result<(), Box<dyn std::error::Error>> {
		...
		info!("{SERVER_NAME} v{SERVER_VERSION} starting on stdio");
	...
	async fn handle_tools_call(&self, id: Value, params: Value) -> JsonRpcResponse {
		...
		let result = tools::call_tool(&self.engine, &name, arguments).await;
	...
	async fn handle_resources_read(&self, id: Value, params: Value) -> JsonRpcResponse {
		...
		match resources::read_resource(&self.engine, &uri).await {
	...
	async fn handle_prompts_get(&self, id: Value, params: Value) -> JsonRpcResponse {
		...
		match prompts::get_prompt(&self.engine, &name, &arguments).await {

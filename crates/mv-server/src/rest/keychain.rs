...
async fn run_post_unseal_maintenance(
    ...
    if post_unseal_failpoint_enabled(state.as_ref(), "MINDVAULT_TEST_FAIL_POST_UNSEAL_MIGRATE") {
    ...
    if post_unseal_failpoint_enabled(state.as_ref(), "MINDVAULT_TEST_FAIL_POST_UNSEAL_REBUILD") {
...
pub async fn unseal_vault(
    ...
    let unseal_result: Result<(), MvError> = if body.from_secure_enclave {
        ...
        {
            log_and_record_unseal_failure(&state, subject, method, "secure_enclave_requires_macos")
                .await?;
            return Err((

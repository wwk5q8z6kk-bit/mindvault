...
- **Status:** Superseding master plan (research-expanded)
- **Date:** 2026-07-26 · **Revised:** 2026-07-31
- **Working product name:** MindVault Next
- **Internal architecture name:** Sovereign Context Fabric
- **Owner:** Hassan El Jesr
- **Sources:** Desktop research-expanded master plan; baseline 2026-07-26 plan; research expansion delta; adjacent-domain research register (see [`docs/research/`](research/))

This document supersedes [`DEVELOPMENT_PLAN.md`](../DEVELOPMENT_PLAN.md) for product direction and roadmap narrative. It does not discard the current repository. It reclassifies the repository as a valuable implementation baseline that must be audited, preserved, migrated, and evolved.

## Authority and conflict rules

1. This document **supersedes** `DEVELOPMENT_PLAN.md` for product direction and program narrative.
2. Where this document and a **ratified** constitution or ADR conflict on an already-decided boundary, the ratified document wins until a dated ADR updates it:
   - [`INTEROPERABILITY_CONSTITUTION.md`](../INTEROPERABILITY_CONSTITUTION.md)
   - ADRs 010–016 under [`docs/adr/`](adr/)
   - Existing contracts under [`docs/architecture/`](architecture/)
3. **Execution status** is never inferred from prose here. Status lives only in [`IMPLEMENTATION_BACKLOG.md`](../IMPLEMENTATION_BACKLOG.md) (`verified` requires evidence).
4. Evidence-first: distinguish verified implementation from claims; filenames and “code present” are not completion.
5. Sections **102–145** (research expansion) are binding where earlier examples appear narrower. Examples are conformance floors, never scope ceilings.
...
|------|------------|--------------|----------------|
| Interoperability constitution + ADR 011 | Phase 0–1 | Ratified; supporting contracts present | DOC, IK |
| Kernel: IDs, events, outbox, grants, receipts, inbox | Phase 1 | Implemented kernel slices; ADRs 013–016 | IK (partial verified) |
| WorkOrder / AgentRun / Artifact | Phase 13 | ADR 012 accepted; migration 038; **internal only** — no external dispatcher/provider/A2A completion | AGENT |
| Knowledge workspace / Obsidian | Phase 7 | ADR 008–009; migration 031; Stage 1–2 path | WS |
| Collaborative Spaces productization | Phase 10–11 | ADR 010 Proposed; baseline audit exists | SPACE |
| Bidirectional MCP + A2A | Phase 9 | Local MCP server exists; dual-role + A2A incomplete | PROTO |
| Connector runtime / universal gateway | Phase 8 | Adapters and MCP marketplace registry exist; manifest v2 runtime incomplete | EXT, SRC |
| Federation production | Phase 15 | Experimental transport; **production disabled** per `FEDERATION_THREAT_MODEL.md` | FED |
| Personal OS surfaces | Domain packs | Goals, habits, calendar, voice, flashcards, clipper, focus **in-tree** | — (UX; pack governance TBD) |
| Vertical / proof portfolio | §74–78, §141–142 | Foundation proofs **not** complete as production paths | SLICE |

Roadmap phases → backlog groups: `IK`, `WS`, `SPACE`, `AGENT`, `PROTO`, `SRC`, `SLICE`, `FED`, `EXT`, `PORT`, `HYG`, `DOC`, `PARK`.
...
> **Formula:** Private Vaults + Shared Spaces + Evidence-backed Knowledge + Structured Work + Agent Delegation + Universal Interoperability + Trust and Human Authority
...
> **Formula:** Universal Capability Kernel + Versioned Domain Packs + Malleable Views and Workflows + Open Protocol Profiles + Conformance and Unknown-Domain Tests
...
&Sovereign Personal Vaults + &Governed Shared Spaces + &Typed Evidence and Temporal Knowledge + &Universal Capability Algebra + &Composable Domain Packs + &Malleable Views and End-User Programming + &Durable Workflows and Human Tasks + &Human, Device, Application, and Agent Identity + &Policy, Consent, Purpose, and Delegation + &Structured Work and Agent Runs + &Bidirectional MCP and A2A Interoperability + &Open Connectors, Extensions, and Alternate Clients + &Federated Context Nodes and Query-to-Data + &Provenance, Lineage, Attestation, and Trust Ledger + &Resource, Cost, Energy, and Attention Governance + &Records Continuity, Recovery, and Digital Legacy + &Scenario Branching and Observed/Simulated Separation + &Device, Edge, Geospatial, and Physical-Safety Boundaries + &Moderation, Abuse Handling, and Community Governance + &Human Authority, Portability, and Reversibility
end{aligned}
...
| Document | Path |
|----------|------|
| Adjacent-domain research register | [`docs/research/adjacent-domain-research-register-2026-07-31.md`](research/adjacent-domain-research-register-2026-07-31.md) |
| Research expansion delta | [`docs/research/research-expansion-delta-2026-07-31.md`](research/research-expansion-delta-2026-07-31.md) |
| July 26 baseline master plan | [`docs/research/master-enhancement-plan-2026-07-26-baseline.md`](research/master-enhancement-plan-2026-07-26-baseline.md) |

*Execution status: [`IMPLEMENTATION_BACKLOG.md`](../IMPLEMENTATION_BACKLOG.md). Binding interop law: [`INTEROPERABILITY_CONSTITUTION.md`](../INTEROPERABILITY_CONSTITUTION.md).*

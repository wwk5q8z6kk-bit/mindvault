...
<div class="grid gap-6 lg:grid-cols-12">
	<section class="lg:col-span-5">
			...
			{#if quickAddPreview && quickAddText.trim()}
				...
				>
					...
					{#each quickAddPreview.labels as label}
						<span
							class="rounded bg-[rgb(var(--mv-panel-strong))] px-1.5 py-0.5 text-[rgb(var(--mv-muted))]"
							>#{label}</span
						>
					{/each}
				</div>
		...
		<!-- Task filter view tabs (All/Today/Upcoming) -->
		<div class="mt-3 flex flex-wrap gap-1 rounded-lg border border-[rgb(var(--mv-border))] bg-[rgb(var(--mv-panel))]/70 p-1">
			{#each [
				{ key: 'all', label: 'All' },
				...
				<button
					class="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition {view === tab.key
						? 'bg-[rgb(var(--mv-panel-strong))] text-[rgb(var(--mv-text))] shadow-sm'
						: 'text-[rgb(var(--mv-muted))] hover:text-[rgb(var(--mv-text))] hover:bg-[rgb(var(--mv-panel-strong))]/30'}"
					on:click={() => applyFilter({ view: tab.key as TaskView, status: 'all' })}
				>
					{tab.label}
				</button>
			{/each}
...
{#if showDeleteConfirm}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
		<div
			role="alertdialog"
			class="w-full max-w-sm rounded-xl border border-[rgb(var(--mv-border))] bg-[rgb(var(--mv-panel))] p-6 shadow-xl"
		>

...
<div class="mx-auto max-w-6xl space-y-6 p-6">
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="text-sm text-[rgb(var(--mv-muted))]">
				Governed agent execution: declared scope, run attempts, gate evidence, and artifacts. Live
				updates arrive on the agent stream when your session is not namespace-scoped.
			</p>
		</div>
		<button
	...
	{:else if !workOrders.length}
		<p
			class="rounded-xl border border-[rgb(var(--mv-border))] bg-[rgb(var(--mv-panel))]/40 p-6 text-sm text-[rgb(var(--mv-muted))]"
		>
			No work orders yet. A proposal becomes schedulable only once its declared write scope resolves
			against a Tool Grant.
		</p>
	{:else}
		<div class="grid gap-4 md:grid-cols-[18rem_1fr]">
			...
			{#if selected}
				<div class="space-y-4">
					<section
						class="rounded-xl border border-[rgb(var(--mv-border))] bg-[rgb(var(--mv-panel))]/40 p-4"
					>
						...
						{#if selected.non_goals.length}
							<p class="mt-2 text-xs text-[rgb(var(--mv-muted))]">
								<span class="font-medium">Non-goals:</span>
								{selected.non_goals.join('; ')}
							</p>
						{/if}
								...
								<div class="mt-3">
									...
									{#if gatesByRun[run.run_id]?.length}
										...
										<p class="mt-2 text-[11px] text-[rgb(var(--mv-muted))]/80">
											Evidence is immutable. A run can never satisfy its own independent review, and
											a failed gate is not re-rolled — recovery is a new attempt.
										</p>
									{:else}
